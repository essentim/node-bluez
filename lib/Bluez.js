...
class Bluez extends EventEmitter {
    ...
    async getDevice(address) {
        let path = address;
        if (!path.match(new RegExp("^/org/bluez/(\\w+)/dev_(\\w+)$"))) {
          path = Object.keys(this.devices).find(otherPath => {
            const match = otherPath.match(new RegExp("^/org/bluez/(\\w+)/dev_(\\w+)$"));
            return match[2] === address || match[2].replace(/_/g, ":") === address;
          });
        }
        if (!path) throw new Error("Device not found");
        if (this.devices[path]) {
            // Device already created
            return this.devices[path];
        }
        const interface_ = await this.getInterface('org.bluez', path, 'org.bluez.Device1');
        ...
        this.devices[path] = new Device(interface_, changed);
        return this.devices[path];
    }
