...
class Device extends EventEmitter {
    constructor(_interface, propsInterface) {
        super();

        this._interface = _interface;
        this._interfaceProps = propsInterface;
        this.services =  {};
        this.onPropertyChanged = this.onPropertyChanged.bind(this);
        this._interfaceProps.on('PropertiesChanged', this.onPropertyChanged);
    }

    onPropertyChanged(intf, props, invalidatedProps) {
        if (intf !== "org.bluez.Device1") return;
        if (props.ManufacturerData) {
            this.emit("manufacturerDataChanged", props.ManufacturerData);
        }
        if (props.RSSI) {
            this.emit("rssiChanged", props.RSSI);
        }
    }
